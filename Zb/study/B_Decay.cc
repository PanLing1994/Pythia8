...
int main() {
  ...
  int nEvent = 100;
  int nListRedo = 1;//redoHadron and redoBDecay

  // Number of times decays/hadronization should be redone for each event.
  int nRepeat = 10;                               //decay 10 times
  if (!redoBDecays && !redoHadrons) nRepeat = 1;  //only find the BDecay and Hadron Decay, other decay channel we dont care.
  ...
  pythia.readString("HardQCD:gg2bbbar = on");          // You would need to use HardQCD:all or even SoftQCD:nonDiffractive for that.
  pythia.readString("HardQCD:qqbar2bbbar = on");
  pythia.readString("PhaseSpace:pTHatMin = 50.");     // Simulate b production above given pTmin scale.
  ...
  int bCodes[28] = {511, 521, 531, 541, 5122, 5132, 5142, 5232, 5242,
    ...
    5514, 5522, 5524, 5532, 5534, 5542, 5544, 5544 };     //all B hadron decay possible 28 finial states
  int nCodes = 28;

  // Repeated decays: location of B handrons.
  vector<int> iBHad; //B hadron location.
  int nBHad = 0; 

  // Repeated hadronization: spare copy of event.
  Event savedEvent;  //copy event.

  if (redoHadrons) pythia.readString("HadronLevel:all = off");    // Repeated hadronization: switch off normal HadronLevel call.

  pythia.init();         // Initialize for LHC energies; default 14 TeV
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    if (redoBDecays) for (int iC = 0; iC < nCodes; ++iC)
      pythia.particleData.mayDecay( bCodes[iC], false);             // Repeated decays: switch off decays of weakly decaying B hadrons.
    ...
    nBperEvent.fill( nBquark );    //fill in b quark per-event into histogram.
    ...
    if (redoBDecays) {
      ...
      for (int i = 0; i < event.size(); ++i) {
        ...
        if (idAbs == bCodes[iC]) {
          iBHad.push_back(i);   //if BDecay finial particles in bCodes save it.
          break;
        }
      }
      ...
      event.saveSize();         // Repeated decays: store size of current event.

      // Repeated decays: switch back on weakly decaying B hadrons.
      for (int iC = 0; iC < nCodes; ++iC)
        pythia.particleData.mayDecay( bCodes[iC], true);           // switch on weakly decaying B hadrons.
    ...
    for (int iRepeat = 0; iRepeat < nRepeat; ++iRepeat) {
      ...
      if (redoBDecays) {
        if (iRepeat > 0) {
          event.restoreSize();     //copy all decay information

          // Repeated decays: mark decayed B hadrons as undecayed.
          for (int iB = 0; iB < nBHad; ++iB) event[ iBHad[iB] ].statusPos();   //undecayed B hadron
        }
      ...
      if (nMuNeg + nMuPos > 1) {
        ++nWithPair;              //must lager 2 muon particles will form a pair.
